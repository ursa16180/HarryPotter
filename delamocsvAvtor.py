...
#vzorec_povprecna_ocena ###TODO pomoje ne obstaja
...
for avtor in avtorji:
    ...
    print(avtor)
    for vzorec1 in re.finditer(vzorec_ime, vsebina):
        podatki1 = vzorec1.groupdict()
        #print(podatki1)
    for vzorec2 in re.finditer(vzorec_kraj_datum_rojstva,vsebina):
    ...
    podatkiAvtor['kraj_rojstva']=podatki2['kraj_rojstva']
    if podatki2['leto'] is not None:
        podatkiAvtor['datum_rojstva']=uredi_datum(podatki2['mesec'],podatki2['dan'],podatki2['leto'])
    else:
        podatkiAvtor['datum_rojstva'] = None
    print(podatkiAvtor)
    seznam_vseh_avtorjev += [podatkiAvtor]
    orodja.zapisi_tabelo(seznam_vseh_avtorjev, ['id', 'ime', 'datum_rojstva', 'kraj_rojstva'],
